<?php
/**
 * Plugin Name: LSX Give PayFast Gateway
...
function lsx_payfast_load_textdomain() {
	load_plugin_textdomain( 'payfast_lsx', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
}
...
function payfast_process_payment( $purchase_data, $recurring = false ) {
	...
	$payment_data = array(
		...
		'lsx_form_title' => $purchase_data['post_data']['give-form-title'],
		'lsx_form_id'    => $purchase_data['post_data']['give-form-id'],
	...
	$required     = array(
		'lsx_first' => __( 'First Name is not entered.', 'payfast_give' ),
		'lsx_last'  => __( 'Last Name is not entered.', 'payfast_give' ),
	);
	...
	if ( $errors ) {
		...
		$payfast_args .= '&merchant_key=' . $lsx_options['payfast_key'];
		$payfast_args .= '&return_url=' . urlencode( apply_filters( 'lsx_success_page_redirect', $redirect, 'payfast', $query_string ) );
		$payfast_args .= '&cancel_url=' . urlencode( $cancelurl );
		$payfast_args .= '&notify_url=' . urlencode( trailingslashit( home_url() ) );
		$payfast_args .= '&name_first=' . $purchase_data['post_data']['lsx_first'];
		$payfast_args .= '&name_last=' . $purchase_data['post_data']['lsx_last'];
		$payfast_args .= '&email_address=' . urlencode( $purchase_data['post_data']['lsx_email'] );
		...
		if ( isset( $lsx_options['payfast_pass_phrase'] ) ) {
...
add_filter( 'lsx_settings_gateways', 'payfast_add_settings' );
